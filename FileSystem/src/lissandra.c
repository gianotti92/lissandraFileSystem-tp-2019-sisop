...
void* TH_compactacion(void* p){
	...
	while(item->endFlag == 0){
		...
		if(compac_delete_tmpc_files(tmpc_files)!=0) {
			...
			log_error(LOG_ERROR,"Compactacion: Error al borrar los archivos de la tabla %s, se bloqueo por %lu milisegundos",item->tableName,endBlocking-startBlocking);
			continue;
		}
		...
		if(compac_save_partition_registers(registrosParticiones,item->tableName)!=0) {
			...
			pthread_rwlock_unlock(&fnd->lock);
			long endBlocking = getTimestamp();
			log_error(LOG_ERROR,"Compactacion: Error al crear las nuevas particiones de la tabla %s, se bloqueo por %lu milisegundos",item->tableName,endBlocking-startBlocking);
			continue;
		}

		pthread_rwlock_unlock(&fnd->lock);
		long endBlocking = getTimestamp();
		log_info(LOG_INFO,"Se ha compactado la tabla %s, se bloqueo por %lu milisegundos",item->tableName,endBlocking-startBlocking);
